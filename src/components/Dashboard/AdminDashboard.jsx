...
import { FiUsers, FiGrid, FiCreditCard, FiMenu, FiX } from "react-icons/fi";
...
const AdminDashboard = () => {
  ...
  if (loading || !overview)
    return <p className="text-center mt-10 text-white">Loading Admin Dashboard...</p>;
  ...
  return (
    <div className="relative min-h-screen flex bg-gradient-to-br from-[#0a0d15] via-[#090c14] to-[#070a12] text-gray-200">
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-screen w-64 bg-white/5 backdrop-blur-xl border-r border-white/10 p-6 flex flex-col shadow-xl z-50 transform transition-transform duration-300
          ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="flex items-center justify-between mb-10">
          <img src={Logo} alt="logo" className="w-10" />
          <button className="lg:hidden text-white text-xl" onClick={() => setSidebarOpen(false)}>
            <FiX />
          </button>
        </div>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen pt-16 lg:pt-0">
        ...
        <div className="flex-1 p-6 sm:p-8 lg:p-10 space-y-6 overflow-y-auto">
          {/* Overview */}
          {activeTab === "overview" && (
...
{activeTab === "users" && (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <div className="overflow-x-auto rounded-2xl bg-white/5 border border-white/10 shadow-lg p-4">
            ...
            <tr key={u._id} className="border-b border-white/10">
              ...
              <td className="px-4 py-2">
                <select
                  ...
                  className="bg-[#1a1a26] px-2 py-1 rounded-md text-white"
                >
                ...
                {clubs.map((c) => (
                  ...
                  >
                    ...
                    <div className="flex gap-3 mt-2">
                      {c.status === "pending" && (
                        <>
                          <button onClick={() => handleApproveClub(c._id)} className="btn-primary text-sm">
                            Approve
                          </button>
                          <button onClick={() => handleRejectClub(c._id)} className="btn-delete text-sm">
                            Reject
                          </button>
                        </>
                      )}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {payments.map((p) => (
                  ...
                  >
                    ...
                    <p className="opacity-60 text-sm">{p.userEmail || p.userId?.email}</p>
